import type { Character, ActiveEffect, Weather, Season, TimeOfDay, WeatherEffect, TimeOfDayEffect } from "$lib/types/character";
import type { GameData } from "$lib/services/gameDataService";
import { processCharacterTurn } from '$lib/ai/brain';
import { decayTick } from '$lib/ai/fatigue';
// Future: wire simple policy here by swapping determineNextAction in brain or calling policy
import { allDivinities } from "$lib/data/divinities";
import { getFallbackThought } from "$lib/data/thoughts";
import type { OutboxChronicleEntry } from "$lib/types/chronicle";
...
export interface EngineServices {
    ...
    listInteractions: (charId: string, limit: number) => Promise<any[]>;
    evaluateAchievements: (char: any, data: any) => any[];
    persistAchievementUnlocks: (userId: string, char: any, unlocks: any[]) => Promise<void>;
}
        ...
        for (const effect of updatedChar.effects) {
            ...
            if (effect.id === 'disease_vampirism' || effect.id === 'disease_lycanthropy') {
                ...
                if (drain > 0) {
                    ...
                    const hungerKey = 'hunger:spam:log';
                    const lastHungerLogAt = Number((updatedChar.actionCooldowns as any)?.[hungerKey] || 0);
                    if (now - lastHungerLogAt > 6 * 60 * 1000 && Math.random() < 0.5) {
                        logs.push(`Голод подтачивает силы ${updatedChar.name}.`);
                        if (!updatedChar.actionCooldowns) updatedChar.actionCooldowns = {} as any;
                        updatedChar.actionCooldowns[hungerKey] = now;
                    }
                }
...
async function processActionCompletion(character: Character, gameData: GameData, userId: string): Promise<{char: Character, logs: string[], chronicles: OutboxChronicle[]}> {
    ...
    switch (currentAction.type) {
        ...
        case 'quest':
            ...
            try {
                const { getQuest, completeQuest, applyRewardsToCharacter, setTaskStatus } = await import('$lib/services/questService');
                ...
                if (dbQuest && dbQuest.quest) {
                    ...
                    if (allCompleted || tasks.length === 0) {
                        ...
                        try {
                            const { autoSelectNextQuest } = await import('$lib/services/questService');
                ...
                if (process.env.LOGS === 'true') console.log('[game-engine] Quest service unavailable, trying static quests:', err);
            }
...
function processMood(character: Character): {char: Character, logs: string[]} {
    ...
    // Apply weather mood modifier (probabilistic: 10% chance per tick)
    if (weatherEffect.moodModifier !== 0 && Math.random() < 0.1) {
    ...
    if (updatedChar.status !== 'in-combat') {
        if (updatedChar.mood > 51 && Math.random() < 0.2) {
            updatedChar.mood -= 1;
            moodChanged = true;
        } else if (updatedChar.mood < 49 && Math.random() < 0.2) {
            // Accelerate recovery if mood is very low
            const recoveryAmnt = (updatedChar.mood < 20) ? 3 : 1;
            updatedChar.mood += recoveryAmnt;
            moodChanged = true;
        }
    }
    ...
    if (updatedChar.stats.health.current < updatedChar.stats.health.max * 0.4) {
        if (Math.random() < 0.02) { // 2% chance per tick
            updatedChar.mood = Math.max(0, updatedChar.mood - 1);
            if (Math.random() < 0.2) { // 20% chance to log when penalty applies
...
export async function processGameTick(
    ...
    // Auto-heal malformed or corrupted states (NaN/null from older bugs)
    if (typeof updatedChar.mood !== 'number' || isNaN(updatedChar.mood) || updatedChar.mood === null) {
        updatedChar.mood = 50; // Give a baseline mood to cure the systemic static bug
    }
    if (typeof updatedChar.stats.health.current !== 'number' || isNaN(updatedChar.stats.health.current) || updatedChar.stats.health.current === null) {
        updatedChar.stats.health.current = updatedChar.stats.health.max;
    }
    ...
    // --- 2. POWER REGENERATION (real-time based: 100% recovery in 15 minutes) ---
    try {
        refreshInterventionPower(updatedChar);
    } catch (e) {
        console.error('Error regenerating power:', e);
    }
    ...
    if (shouldTakeTurn) {
        ...
        if (services?.getPendingDivineMessages && services?.markDivineMessageProcessed && services?.selectDivineReaction) {
            try {
                const pending = await services.getPendingDivineMessages(updatedChar.id, 3);
                for (const m of pending as any[]) {
                    const text: string = (m as any).text || '';
                    if (!text) continue;
                    
                    // Select appropriate reaction based on character state
                    const reaction = services.selectDivineReaction({
                    ...
                    await services.markDivineMessageProcessed((m as any).id);
                }
            } catch (err) {
    ...
    if (services?.listInteractions) {
        try {
            const interactions = await services.listInteractions(userId, 5); // Get last 5 interactions
    ...
    if (services?.evaluateAchievements && services?.persistAchievementUnlocks) {
        try {
            ...
            const unlocks = services.evaluateAchievements(updatedChar, gameData);
            if (unlocks.length > 0) {
                await services.persistAchievementUnlocks(userId, updatedChar, unlocks);
